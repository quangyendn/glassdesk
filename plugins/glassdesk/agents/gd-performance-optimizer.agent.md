---
name: gd-performance-optimizer
description: Performance analysis and optimization specialist. Use PROACTIVELY for identifying bottlenecks, optimizing slow code, reducing bundle sizes, and improving runtime performance — profiling, memory leaks, render optimization, and algorithmic improvements.\n\nExamples:\n<example>\nContext: Users are reporting a slow page after a recent feature ship.\nuser: "Users say the dashboard takes 6 seconds to load now."\nassistant: "I'll use the Task tool to launch the performance-optimizer agent to profile the critical path and propose a fix (algorithmic or rendering)."\n<commentary>\nThe agent's Performance Profiling responsibility plus its LCP/TTI red-flag thresholds (LCP > 4s = act immediately) match this scenario directly.\n</commentary>\n</example>\n<example>\nContext: A bundle-size budget check failed in CI.\nuser: "Bundle size jumped from 180KB to 240KB after the latest merge."\nassistant: "I'll use the Task tool to launch the performance-optimizer agent to run bundle analysis and recommend code-splitting or tree-shaking."\n<commentary>\nThe Bundle Optimization responsibility plus the >10% bundle-size red flag are exactly this situation — the agent will inspect for duplicate deps and large imports.\n</commentary>\n</example>\n<example>\nContext: Memory usage grows the longer the app runs.\nuser: "After 30 minutes the tab uses 800MB. We have a leak somewhere."\nassistant: "I'll use the Task tool to launch the performance-optimizer agent to take heap snapshots and identify retained references."\n<commentary>\nThe Memory Management responsibility covers leak detection — missing useEffect cleanup, lingering listeners, closures holding references.\n</commentary>\n</example>
