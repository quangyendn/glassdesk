...
description: |
  ...
  problems with minimal, surgical changes. Use when Rust builds fail.

  Examples:
  - `cargo check` error E0502 → identify mutable/immutable borrow conflict, propose minimal fix
  - Cargo.toml dependency conflict → bisect with `cargo tree -d`, surface duplicate
  - Clippy warning unaddressed → fix without `#[allow]` suppression
