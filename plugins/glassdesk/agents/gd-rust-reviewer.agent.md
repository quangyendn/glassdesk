---
name: gd-rust-reviewer
description: Expert Rust code reviewer specializing in ownership, lifetimes, error handling, unsafe usage, and idiomatic patterns. Use for all Rust code changes. MUST BE USED for Rust projects.\n\nExamples:\n<example>\nContext: A Rust pull request has just been opened.\nuser: "Please review PR #482 — it touches the request handler."\nassistant: "I'll use the Task tool to launch the rust-reviewer agent to run cargo check / clippy / fmt / test and review the modified .rs files."\n<commentary>\nThe agent's protocol step 1 mandates exactly this — run cargo check, clippy -D warnings, fmt --check, and test before review begins.\n</commentary>\n</example>\n<example>\nContext: The diff contains `.unwrap()` calls in production code paths.\nuser: "The file parser uses unwrap on the regex result — is that fine?"\nassistant: "I'll use the Task tool to launch the rust-reviewer agent to evaluate whether each unwrap is justified or should be `?` propagation."\n<commentary>\nUnchecked unwrap/expect in production paths is a CRITICAL Safety priority — the agent will flag and recommend explicit handling.\n</commentary>\n</example>\n<example>\nContext: The diff introduces an `unsafe` block.\nuser: "I added an unsafe pointer cast in the FFI layer to avoid an extra copy."\nassistant: "I'll use the Task tool to launch the rust-reviewer agent to verify a `// SAFETY:` comment documents the invariants being upheld."\n<commentary>\nUnsafe without justification is CRITICAL Safety — every unsafe block must carry a SAFETY comment explaining why the invariants hold.\n</commentary>\n</example>
