...
When the `gd-planner` agent is available, the **main thread orchestrates only**:
- Read inputs (spec, scout/researcher reports, codebase docs)
- Dispatch `gd-researcher` agents in parallel for investigation
- Dispatch `gd-planner` agent for plan synthesis (it writes the actual plan files)
- Verify the plan files exist and surface the path

The main thread MUST NOT write `plan.md` or `phase-*.md` directly when `gd-planner` is dispatchable. This avoids 2x premium-tier token spend (orchestrator Opus + planner Opus both writing). If `gd-planner` agent is unavailable for any reason, fall back to writing inline and surface a warning.
...
4. **Update on create**: Run `node "$GD_PLUGIN_PATH/scripts/set-active-plan.cjs" {plan-dir}`
