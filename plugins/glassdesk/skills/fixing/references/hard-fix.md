...
3. Call `gd-debugger` subagent: "Find root cause of: [issue description]". Wait for report.
4. If root cause requires external research: call `gd-researcher` subagent to find solutions/patterns online.
5. Call `gd-planner` subagent: "Create implementation plan to fix: [root cause summary]. Research: [researcher report path]".
6. Execute `/code` SlashCommand to implement the plan step by step.
7. Final report: summary of changes, explanation, how to get started, next steps. Ask if user wants to commit/push (if yes, use `gd-git-manager`).
