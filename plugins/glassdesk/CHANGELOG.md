...
- **Double context injection under a dual install** — when both the marketplace plugin and an `npx glassdesk init` install are active, Claude Code runs `dev-rules-reminder.cjs` twice per prompt and concatenates both `additionalContext` values. The hook now claims a per-prompt lock file in `$TMPDIR` (keyed on session id + cwd + prompt, 30s TTL); the losing copy exits silently. Fails open — any error in the guard emits the context rather than suppressing it.
