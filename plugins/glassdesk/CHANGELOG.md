...
- **Double context injection under a dual install** — when both the marketplace plugin and an `npx glassdesk init` install are active, Claude Code runs `dev-rules-reminder.cjs` twice per prompt and concatenates both `additionalContext` values. The hook now claims a per-prompt lock file in `$TMPDIR`, keyed on session id + cwd + prompt plus a monotonic 10s time bucket; the losing copy exits silently. Bucket names never repeat, so there is no expired lock to reclaim — the race that in-place reclamation invites — and a copy that wins a bucket boundary stands down if the previous bucket was claimed within the last second. Locks older than 5 minutes are swept. The guard fails open — any error in it emits the context rather than suppressing it — and is skipped entirely when the payload carries no `transcript_path` (Codex, which loads only the plugin manifest and so has no duplicate). **This only suppresses the duplicate when both installed copies are on this version or newer**; during an upgrade skew the older copy still injects.
